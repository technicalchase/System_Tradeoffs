...
print("="*80)
print("APPLYING REDACTION TO ENTIRE DATASET")
print("="*80)

def apply_redaction_to_dataset(df, redactor, redactor_name, max_samples=None):
    """
    Apply redaction to the entire dataset and return results.
    """
    print(f"\nApplying {redactor_name} redaction...")

    if max_samples:
        sample_df = df.head(max_samples)
        print(f"Processing {len(sample_df)} samples (limited for demo)")
    else:
        sample_df = df
    ...
    for idx, row in sample_df.iterrows():
        try:
            ...
            results.append({
                ...
                'ground_truth_pii': row['pii_parsed'],
...
# Apply all redaction modes to full dataset
print("Processing full dataset...")

# Strict mask redaction
strict_results = apply_redaction_to_dataset(df, strict_redactor, "Strict Mask")

# Partial mask redaction
partial_results = apply_redaction_to_dataset(df, partial_redactor, "Partial Mask")

# LLM mask redaction (if available)
if llm_available:
    llm_results = apply_redaction_to_dataset(df, llm_redactor, "LLM Mask")
else:
    llm_results = []

# Display sample results
print("\n" + "="*80)
print("SAMPLE REDACTION RESULTS")
print("="*80)

sample_idx = 0
print(f"\nSample {sample_idx + 1}:")
print(f"Original: {strict_results[sample_idx]['original_text']}")
print(f"Strict:   {strict_results[sample_idx]['redacted_text']}")
print(f"Partial:  {partial_results[sample_idx]['redacted_text']}")
if llm_results:
    print(f"LLM:      {llm_results[sample_idx]['redacted_text']}")

# Create comparison DataFrame
comparison_data = []
for i in range(len(strict_results)):
    comparison_data.append({
        'sample_id': i + 1,
        'subject_id': strict_results[i]['subject_id'],
        'original_text': strict_results[i]['original_text'],
        'strict_redacted': strict_results[i]['redacted_text'],
        'partial_redacted': partial_results[i]['redacted_text'],
        'llm_redacted': llm_results[i]['redacted_text'] if llm_results else "N/A",
        'ground_truth_pii': strict_results[i]['ground_truth_pii']
    })

comparison_df = pd.DataFrame(comparison_data)

print(f"\nComparison DataFrame created with {len(comparison_df)} samples")
print("\nFirst few rows:")
print(comparison_df[['sample_id', 'subject_id', 'strict_redacted', 'partial_redacted', 'llm_redacted']].head(3))
